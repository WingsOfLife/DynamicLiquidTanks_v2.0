...
public class DuctModel extends ModelBase
{
    ...
    public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
    {
        ...
        if (duct.isBlockThere(directionValues.EAST))
        {
            Left.render(f5);
        }

        if (duct.isBlockThere(directionValues.WEST))
        {
            Right.render(f5);
        }

        if (duct.isBlockThere(directionValues.NORTH))
        {
            Front.render(f5);
        }

        if (duct.isBlockThere(directionValues.SOUTH))
        {
            Back.render(f5);
        }

        if (duct.isBlockThere(directionValues.DOWN))
        {
            Top.render(f5);
        }

        if (duct.isBlockThere(directionValues.UP))
        {
            Down.render(f5);
        }
    }
