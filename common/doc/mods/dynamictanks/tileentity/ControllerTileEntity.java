...
import doc.mods.dynamictanks.helpers.InventoryHelper;
			...
			if (worldObj.getBlockTileEntity(x, y, z) instanceof IInventory) {
				...
				for (int i = 0; i < toFill.getSizeInventory(); i++) {
					...
					if (FluidContainerRegistry.isEmptyContainer(toFill.getStackInSlot(i)) && fillingFrom != null) {
						ItemStack filledAlt = InventoryHelper.getFilledAlternative(toFill.getStackInSlot(i), fillingFrom);
						if (InventoryHelper.inventoryHasRoom(toFill) && fillingFrom.amount - FluidContainerRegistry.BUCKET_VOLUME >= 0) {
							drain(ForgeDirection.UNKNOWN, FluidContainerRegistry.BUCKET_VOLUME, true);
							ItemStack testing = toFill.getStackInSlot(i);
							toFill.setInventorySlotContents(i, InventoryHelper.consumeItem(testing));
							InventoryHelper.insertItem(filledAlt, toFill);
							return;
						}
					}
				}
