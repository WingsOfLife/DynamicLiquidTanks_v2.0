...
public class WorldGenCreepyTree extends WorldGenNewTreeBase {

	private enum TreeBlock {
		LEAVES(new ItemStack(BlockManager.BlockHL, 1, 1)), TRUNK(new ItemStack(BlockManager.BlockHW, 1, 1));
	...
	public WorldGenCreepyTree(boolean par1) {
		super(par1);
	}
	...
	private boolean checkTree(World world, Random rand, int x, int y, int z) {
		...
		int heightID = world.getBlockId(x, y - 1, z);
		if (heightID != BlockManager.BlockHD.blockID || y >= 256 - height - 4) return false;
	...
	private boolean generateTree(World world, Random rand, int x, int y, int z) {
		...
		// Make sure that a tree can grow on the soil
		if (below != BlockManager.BlockHD.blockID || y >= 256 - height - 4) return false;
