...
public class HauntedTreeGenerator extends WorldGenerator
{
    public final boolean seekHeight;
    public final int seaLevel = 64;

    public HauntedTreeGenerator(boolean notify)
    {
        super(notify);
        seekHeight = !notify;
    }
    ...
    int findGround(World world, int x, int y, int z)
    {
        ...
        do
        {
            ...
            if ((heightID == Block.dirt.blockID || heightID == Block.grass.blockID || heightID == Block.sand.blockID || heightID == BlockManager.BlockHD.blockID) && !Block.opaqueCubeLookup[world.getBlockId(x, height + 1, z)])
            {
                ret = height + 1;
                break;
            }
    ...
    public boolean generate(World world, Random par2Random, int x, int y, int z)
    {
        ...
        if (y >= 1 && y + l + 1 <= 128)
        {
                ...
                for (j1 = x - b0; j1 <= x + b0 && flag; ++j1)
                {
                    for (k1 = z - b0; k1 <= z + b0 && flag; ++k1)
                    {
                        if (i1 >= 0 && i1 < 128)
                        {
                            ...
                            {
                                if (l1 != Block.waterStill.blockID && l1 != Block.waterMoving.blockID && l1 == BlockManager.BlockHD.blockID)
                                {
                                    flag = false;
                                }
                                else if (i1 > y)
            ...
            else
            {
                ...
                if ((i1 == Block.grass.blockID || i1 == Block.dirt.blockID || i1 == BlockManager.BlockHD.blockID) && y < 128 - l - 1)
                {
                    ...
                    for (j2 = y - 3 + l; j2 <= y + l; ++j2)
                    {
                        ...
                        for (l1 = x - k1; l1 <= x + k1; ++l1)
                        {
                            ...
                            for (int k2 = z - k1; k2 <= z + k1; ++k2)
                            {
                                ...
                                {
                                    this.setBlockAndMetadata(world, l1, j2, k2, BlockManager.BlockHL.blockID, 0); //TODO
                                }
                            }
                        }
                    }
                    ...
                    for (j2 = 0; j2 < l; ++j2)
                    {
                        ...
                        if (j1 == 0 || (block != null && block.isLeaves(world, x, y + j2, z)) || j1 == Block.waterMoving.blockID || j1 == Block.waterStill.blockID || j1 == BlockManager.BlockHD.blockID)
                        {
                            this.setBlock(world, x, y + j2, z, BlockManager.BlockHW.blockID); //TODO
                        }
                    }
