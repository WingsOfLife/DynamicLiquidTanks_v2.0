...
public class HauntedTreeGenerator2 extends WorldGenerator
{
    ...
    public HauntedTreeGenerator2(boolean notify, int treeHeight, int treeRange, int woodMeta, int leavesMeta)
    {
        super(notify);
    ...
    public boolean generate(World world, Random random, int xPos, int yPos, int zPos)
    {
        ...
        if (yPos >= 1 && yPos + height + 1 <= 256)
        {
            ...
            boolean isSoil = true;//(soil != null && soil.canSustainPlant(world, xPos, yPos - 1, zPos, ForgeDirection.UP, NContent.rareSapling));
            ...
            for (int x = xPos - i2; x <= xPos + i2; ++x)
            {
                ...
                for (int z = zPos - i2; z <= zPos + i2; ++z)
                {
                    ...
                    if (Math.abs(k2) != i2 || Math.abs(i3) != i2 || random.nextInt(2) != 0 && k1 != 0)
                    {
                        ...
                        {
                            this.setBlockAndMetadata(world, x, y, z, BlockManager.BlockHL.blockID, this.metaLeaves);
                        }
                    }
                }
            }
    ...
    void placeTrunk(World world, int xPos, int yPos, int zPos, int height)
    {
        for (int localHeight = 0; localHeight < height; ++localHeight)
        {
            ...
            {
                this.setBlockAndMetadata(world, xPos, yPos + localHeight, zPos, BlockManager.BlockHW.blockID, this.metaWood);
            }
        }
    }
}
